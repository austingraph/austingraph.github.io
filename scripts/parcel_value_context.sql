...
--
-- Cohort = the up-to-300 nearest parcels sharing this parcel's zoning_base,
-- ordered by centroid distance (uses the parcels_centroid_idx GIST KNN index).
-- Percentile is the share of the cohort at or below this parcel's $/sqft
-- (higher = pricier than its neighbors).
--
...
declare
  ...
  v_bldg    jsonb := null;
  v_landj   jsonb := null;
  v_limit   int   := 300;     -- nearest-N same-zoning comparables
begin
  -- Target parcel: centroid, zoning, value parts, and lot size (TCAD acres if
  -- present, else the geodesic polygon area converted to square feet).
  select centroid, zoning_base, appr_land_val, appr_impr_val, appr_living_sqft,
         coalesce(nullif((metadata->>'tcad_acres')::numeric, 0) * 43560,
                  nullif(st_area(geom::geography) * 10.7639104, 0))
    into v_ctr, v_zoning, v_land, v_impr, v_sqft, v_lotsqft
    from public.parcels
  ...
  if coalesce(v_impr, 0) > 0 and coalesce(v_sqft, 0) > 0 then
    ...
    with cohort as (
      select appr_impr_val::numeric / appr_living_sqft as psf
        from public.parcels
       where zoning_base is not distinct from v_zoning
